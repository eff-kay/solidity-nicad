...
void TProg () {
	...
	TL_TLS_TLSCAT("/bin/rm -rf ", ccdir, __x64);
	{
	    ...
	    TLSTRASS(4095, commandline, __x63);
	};
    };
	...
	TLSTRASS(4095, commandline, __x65);
    };
    rc = system(commandline);
    if (rc != 0) {
	TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
