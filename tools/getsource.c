...
void TProg () {
	...
	TLSTRASS(4095, ccfile, __x74);
    };
    TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
    TL_TLI_TLIPS ((TLint4) 0, ccfile, (TLint2) 0);
    TL_TLI_TLIPK ((TLint2) 0);
	...
	TLSTRASS(4095, rccfile, __x75);
    };
    TL_TLI_TLIOF ((TLnat2) 4, rccfile, &rccf);
    if (rccf == 0) {
	TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
	TL_TLI_TLIPS ((TLint4) 0, "*** Error: can\'t open sourced clone classes output file", (TLint2) 0);
	TL_TLI_TLIPK ((TLint2) 0);
	TL_TLE_TLEQUIT ((TLint4) 1, (char *) 0, 0);
    };
    for(;;) {
	...
	if (TL_TLS_TLSIND(line, "<source") == 1) {
		...
		TLSTRASS(4095, sourceheader, __x76);
	    };
		...
		if (i <= __x79) {
			...
			for(;;) {
			    ...
			    TL_TLI_TLIGSS((TLint4) 4095, line, (TLint2) sf);
			    if (TL_TLI_TLIEOF((TLint4) sf) || (TL_TLS_TLSLEN(line) < 4095)) {
				break;
			    };
			};
			if (i == __x79) break;
