...
% Cross-clone extension, January 2011
...
% crossclones.x pc1file pc2file threshold minclonelines maxcloneslines showsource
var pc1file : string			% name of the system_functions.xml pcs file
var pc2file : string			% name of the new_functions.xml pcs file
...
% Usage message
procedure useerr
    put : 0, "Usage :  crossclones.x pc1file.xml pc2file.xml [ threshold ] [ minclonesize ] [ maxclonesize ] [ showsource ] > clone-pairs-file.xml"
    put : 0, ""
    put : 0, "  e.g.:  crossclones.x linux_functions.xml freebsd_functions.xml 0.2 > linux_freebsd_functions-crossclones.xml"
    put : 0, ""
...
procedure readpcs
	...
	if index (sourceheader, "<source ") not= 1 then
	    put : 0, "*** Error: synchronization error on pc file 1"
	    quit : 1
	end if
...
procedure readnewpcs
    % Try to open our new_functions.xml file
    var npcf: int
    open : npcf, pc2file, get
	...
	% Get the info header <source ... >
	var sourceheader : string
	get : npcf, sourceheader : *
	pc.info := linetable.install (sourceheader)

	if index (sourceheader, "<source ") not= 1 then
	    put : 0, "*** Error: synchronization error on pc file 2"
	    quit : 1
	end if
...
/* Doesn't apply in case of incremental clones
...
procedure findclones 
	...
	if pcs (i).nlines not= 0 then
		...
		% If the second pc is within  our limits (already marked by nlines>0)
		    ...
		    if similarity not= 0 and 	% only interested in cross-clones
			    pcs (i).sys not= pcs (j).sys then
			npairs += 1
...
function getsysteminfo (pcfile : string) : string
    var slashindex := length (pcfile)
    loop
        const pcfchar : char := type (char, pc1file (slashindex))
        exit when slashindex = 0 or pcfchar = '/'
        slashindex -= 1
    end loop
    result pcfile (slashindex + 1 .. *)
end getsysteminfo

function getsystemname (systeminfo : string) : string
    ...
    result systeminfo (1 .. usindex - 1)
end getsystemname

function getgranularity (systeminfo : string) : string
    var usindex := length (systeminfo)
    loop
        const sichar : char := type (char, systeminfo (usindex))
        exit when usindex = 2 or sichar = '_'
    ...
    result systeminfo (usindex + 1 .. index (systeminfo, ".xml") - 1)
end getgranularity

procedure showpairs
    % System information
    var system1info := getsysteminfo (pc1file)
    var system1name := getsystemname (system1info)
    var system2info := getsysteminfo (pc2file)
    var system2name := getsystemname (system2info)
    var granularity := getgranularity (system1info)
...
put : 0, ""
    ...
    put "<clones>"
    put "<systeminfo processor=\"nicad6\" system=\"", system1name, "\" system2=\"", system2name, "\" granularity=\"", granularity, "\" threshold=\"", round (threshold * 100 + epsilon), 
	...
	% report in old-new order
	if pcs (cp.pc1).sys > pcs (cp.pc2).sys then
	    const temp := cp.pc1
	    cp.pc1 := cp.pc2
	    cp.pc2 := temp
	end if
...
body procedure getprogramarguments
    ...
    % Names of the extracted pc XML files
    pc1file := fetcharg(1)
    pc2file := fetcharg(2)

    % Difference threshold
    if nargs > 2 then
	threshold := strreal (fetcharg (3))
    ...
    % Minimum clone size
    if nargs > 3 then
	minclonelines := strint (fetcharg (4))
    ...
    % Maximum clone size
    if nargs > 4 then
	maxclonelines := strint (fetcharg (5))
...
put : 0, "Processing ", pc1file, " vs ", pc2file, " with difference threshold ", round (threshold * 100 + epsilon), 
