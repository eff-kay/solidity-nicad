...
def extrac_code(file_path, classids, save_path):
    ...
    for i in map(str, classids):
        ...
        cs = re.findall(p, f, re.MULTILINE)
        pre_cs = cs
        p = r'^<source[\s\S]*?(?:contract|library|interface)[ ]?([a-zA-Z0-9_]*)[\s\S]*?\{[\s\S]*<\/source>$'
        all_cs = re.findall(p, cs[0], re.MULTILINE)
        print(all_cs)
        # if len(all_fs)==0:
        #     p = r'^<source[\s\S]*?>([\s\S]*?)<\/source>$'
        #     cs = re.findall(p, cs[0], re.MULTILINE)
...
def extract_contract_ids(config):
    ...
    for filepath in original_paths:
        ...
        save_path_pickle = 'duplicates/contract-ids/{}'.format(filepath+'.p')
        extrac_code(complete_file_path, classids, save_path_pickle)
       
        save_path_txt = open('duplicates/contract-ids/{}'.format(filepath+'.txt'), 'w')
        functions = pickle.load(open(save_path_pickle, 'rb'))

        for f in functions:
            if len(f)>0:
                [save_path_txt.write(fi+'\n') for fi in f] 
